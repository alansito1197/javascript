<?php
    ...
    class PedidoDAO {
        ...
        // Crearemos una función que nos ayudará a obtener el ID del último pedido realizado por usuario:
        public static function obtenerIdUltimoPedido() {
            // Nos conectamos a la base de datos:
            $conexion = DataBase::connect();

            // Iremos a buscar el último registro de cada usuario:
            $busqueda_UltimoPedido = "SELECT id_pedido FROM PEDIDO WHERE tipo_usuario = ? ORDER BY id_pedido DESC LIMIT 1";

            // Preparamos la consulta:
            $stmt = $conexion->prepare($busqueda_UltimoPedido);

            // Vinculamos los parámetros:
            $stmt->bind_param("s", $_SESSION['tipo_usuario']);
            ...
            if ($resultado->num_rows > 0) {
                $row = $resultado->fetch_object();
                $idUltimoPedido = $row->id_pedido;
                // Devolvemos el ID del último pedido:
                return $idUltimoPedido;
            } else {
                // Si no se encuentra ningún pedido, devolvemos false o null
                return null;
            }
        }
